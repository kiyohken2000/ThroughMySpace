...
- [ ] フェーズ1：**【最初】ViewpointComponent左右分離の単独検証** → 空間写真選択 → ドームメッシュ展開 → フローティングパネルUI → 視野狭窄 → 色覚異常 → シームレス切り替え → 強度調整 → 症状説明
- [ ] フェーズ2：中心暗点（アイトラ＋スムージング）→ 白内障（Bloom）→ 飛蚊症（アイトラ）→ 網膜色素変性症 → 症状の重ねがけ → 当事者コメント追加 → アプリ内撮影 → App Store申請
