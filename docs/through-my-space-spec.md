...
| 症状 | 想定手法 | 備考 |
|---|---|---|
| 糖尿病性網膜症 | Entity オーバーレイ（赤い出血斑・黒いシミ）+ CIフィルター（部分ぼかし） | 静的フィルターで近似可能、技術的難易度は低い |
| 夜盲症 | CIフィルター（輝度低下 + 彩度除去） | シェーダーとして最もシンプルな部類、強度調整も容易 |
| 羞明（光過敏） | CIフィルター（白内障Bloom効果の強化版 + ハレーション強調） | 白内障で Bloom 実装済み、光源検出との組み合わせ |
| コントラスト感度低下 | CIフィルター（コントラスト調整） | 実質シェーダー1行レベル、他症状との組み合わせ表現にも有用 |
| スターガルト病 | Entity オーバーレイ（中心暗点の派生） | 中心暗点を実装済み、発症年齢・進行パターンの違いを強度で表現 |

**🟡 工夫すれば実装できる（調整が必要）**

| 症状 | 想定手法 | 課題 |
|---|---|---|
| 高度近視 | CIフィルター（周辺ぼかし + 歪み） | 通常の近視との差別化（網膜の引き延ばし感）の表現に工夫が必要 |
| 半盲・四分盲 | Entity オーバーレイ（視野マスク） | 処理自体は単純だが、Vision Pro で「視界の半分が消える」体験を自然に見せる調整が必要 |
| 角膜ジストロフィー | CIフィルター（白濁 + 光の乱反射） | 症状の種類が多く、代表的な表現の選定が必要 |

**🔴 実装が難しい（安全性・コンセプト上の問題）**

| 症状 | 想定手法 | 課題 |
|---|---|---|
| 眼振 | 映像全体のリアルタイム振動 | Vision Pro の没入空間で映像を揺らすと VR酔いのリスクが極めて高い |
| 複視 | 左右目に意図的にずらした映像を表示 | Vision Pro の立体視と競合する可能性、不快感が非常に強い |
| レーバー先天性黒内障 | ほぼ完全な暗転 | 「自分の空間が変容する」コンセプトと相性が悪い（ほぼ真っ暗になる） |
